...
//  testTime
//
//  Created by AllenKwok on 2016/11/4.
//  Copyright © 2016年 AllenKwok. All rights reserved.
//
...
+ (NSDate *)dateAfterDate:(NSDate *)date{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents *componentsToAdd = [[NSDateComponents alloc] init];
    [componentsToAdd setHour:-[date hour]];
    [componentsToAdd setSecond:-[date second]];
    [componentsToAdd setMinute:-[date minute]];
 ...
 * 获取该天开始的时间
 */
- (NSDate *)begintimeOfDate{
    return [NSDate begintimeOfDate:self];
}

+ (NSDate *)begintimeOfDate:(NSDate *)date{
    return [self dateAfterDate:date];
}
...
- (NSString *)begintimeStringOfDate{
    return [NSDate begintimeStringOfDate:self];
}

+ (NSString *)begintimeStringOfDate:(NSDate *)date{
    return [NSString stringWithFormat:@"%ld000",(NSUInteger)[[self begintimeOfDate:date] timeIntervalSince1970]];
}
...
- (NSDate *)lasttimeOfDate{
    return [NSDate lasttimeOfDate:self];
}

+ (NSDate *)lasttimeOfDate:(NSDate *)date{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDateComponents *componentsToAdd = [[NSDateComponents alloc] init];
    [componentsToAdd setHour:-[date hour]+23];
    [componentsToAdd setSecond:-[date second]+59];
    [componentsToAdd setMinute:-[date minute]+59];
 ...
 * 获取该天结束的时间时间戳字符串
 */
- (NSString *)lasttimeStringOfDate{
    return [NSDate lasttimeStringOfDate:self];
}

+ (NSString *)lasttimeStringOfDate:(NSDate *)date{
    return [NSString stringWithFormat:@"%ld000",(NSUInteger)[[self lasttimeOfDate:date] timeIntervalSince1970]];
}
...
+ (NSTimeInterval)getTodayStartTime{
    ...
    NSString *beginDateString = [NSString stringWithFormat:@"%04ld-%02ld-%02ld 00:00:00",year,month,day];
    
    NSDate *beginDate = [fmt dateFromString:beginDateString];
    
    NSTimeInterval interval = [beginDate timeIntervalSince1970];
    return interval;
}
