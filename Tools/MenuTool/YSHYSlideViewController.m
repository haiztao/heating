...
-(void)PanGestureRespond:(UIPanGestureRecognizer *)gesture
{
    ...
    if(gesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        [UIView  animateWithDuration:0.15 animations:^{
            [self.view setFrame:CGRectMake(orignX, 0, tempWidth, kScreenHeight)];
...
-(void)OpenMenu
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self.view setFrame:CGRectMake(self.menuWidth, 0, self.view.frame.size.width, kScreenHeight)];
        [self.view.window insertSubview:self.leftMenu.view atIndex:0];
        //隐藏 半透明覆盖层
        ...
        if (self.mainView == nil) {
            self.mainView = [[UIView alloc]initWithFrame:self.mainVC.view.frame];
            [self.mainVC.view addSubview:self.mainView];
        }
...
-(void)tapGestureRespond:(UITapGestureRecognizer *)tap{
    [self CloseMenu];
