...
- (instancetype)initWithOrigin:(CGPoint)origin Width:(CGFloat)width Height:(CGFloat)height Type:(JGDirectionType)type Color:(UIColor *)color backGroundImageName:(NSString *)imageName
{
    ...
    if (self) {
        ...
        [self.backGoundView addSubview:imageView];
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 7, self.backGoundView.frame.size.width - 5, self.backGoundView.frame.size.height) style:UITableViewStylePlain];
